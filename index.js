...
function compareArrays(arr1, arr2) {
  ...
  const jsonSortedArr1 = JSON.stringify(sortedArr1)
  const jsonSortedArr2 = JSON.stringify(sortedArr2)
  // Використовуємо `console.assert()`, щоб вивести помилку, якщо масиви не однакові.
  console.assert(
    jsonSortedArr1 === jsonSortedArr2,
    'Масиви не однакові!',
  )
}
...
function buildMatrix(size) {
  ...
  for (let i = 0; i < size; i++) {
    ...
    for (let j = 0; j < size; j++) {
      ...
      martrixRow[j] = Math.floor(Math.random() * 10)
    }
...
function countVowelsAndConsonants(word) {
  ...
  const vowel = `aeiou`
  const vowelCounter = 'Голосна'
  const consonantCounter = 'Приголосна'
  ...
  for (const letter of word) {
    ...
    if (vowel.includes(letterLow)) {
      console.count(vowelCounter)
    } else {
      // коректно ще додати третій лічильник символів (не літер), або звіряти з рядком приголосних
      console.count(consonantCounter)
    }
  }
  // Скидаємо лічильники
  console.countReset(vowelCounter)
  console.countReset(consonantCounter)
}
